...
public:
    ...
    struct Access {
        Access() = default;
        Access(T& value, mutex& value_mutex) : ref_to_value(value), access(value_mutex){
        }
        T& ref_to_value;
    ...
    Access GetAccess() {
        // Access guard();
        return Access{ value_, value_mutex_ };
    }
...
void TestProducerConsumer() {
    ...
    for (size_t i = 1; i <= item_count; ++i) {
        common_queue.GetAccess().ref_to_value.push_back((int)i);
    }
