...
public:
    ...
    virtual double Evaluate(std::function<double(Position)> execute) const = 0;
...
public:
    ...
    double Evaluate(std::function<double(Position)> execute) const override {
        double result = .0;
        switch (type_) {
            case Add:
                result = lhs_->Evaluate(execute) + rhs_->Evaluate(execute); break;
            case Subtract:
                result = lhs_->Evaluate(execute) - rhs_->Evaluate(execute); break;
            case Multiply:
                result = lhs_->Evaluate(execute) * rhs_->Evaluate(execute); break;
...
public:
    ...
    double Evaluate(std::function<double(Position)> execute) const override {
        switch (type_) {
            case UnaryPlus: return operand_->Evaluate(execute); break;
            case UnaryMinus: return -operand_->Evaluate(execute); break;
            default:
                assert(false); throw FormulaException(""); break;
        }
    }
...
double FormulaAST::Execute(std::function<double(Position)> execute) const {
    return root_expr_->Evaluate(execute);
}
