...
public:
    ...
    void Process(BudgetManager& budget) const override {
        ...
        budget.AddBulkOperation(GetFrom(), GetTo(), BulkMoneyAdder{ 0.0, day_spend });
    }
...
public:
    PayTax(Date from, Date to, double percent)
        : ModifyQuery(from, to)
        , percent_(percent) {
    }

    void Process(BudgetManager& budget) const override {
        budget.AddBulkOperation(GetFrom(), GetTo(), BulkTaxApplier{ percent_ });
    }
    ...
    public:
        std::unique_ptr<Query> Construct(std::string_view config) const override {
            auto parts = Split(config, ' ');
            double percent = std::stod(std::string(parts[2]));
            return std::make_unique<PayTax>(Date::FromString(parts[0]), Date::FromString(parts[1]), percent);
        }
    };
