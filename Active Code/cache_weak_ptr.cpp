...
public:
    ...
    std::shared_ptr<Value> GetValue(const Key& key) {
        if(auto p = key_value[key].lock()) {
            return p;
        }
        else {
            std::shared_ptr<Value> value = value_factory_(key);
            key_value[key] = value;
            return key_value[key].lock();
        }
    }
private:
    std::unordered_map<Key, std::weak_ptr<Value>> key_value;
...
public:
    ...
    std::shared_ptr<Book> operator()(const std::string& title) {
        ...
        if (store_.count(title) == 0) {
            ...
            ++load_count_;
            Book book(title, store_.at(title));
