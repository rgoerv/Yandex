...
public:
    ...
    // парковок автомобиля
    int GetCount(VehiclePlate car) const {
...
public:
    ...
    int64_t GetCurrentBill(const VehiclePlate& car) const {
        ...
        if(complete_parks_.count(car) > 0){
            price += chrono::duration_cast<chrono::seconds>(complete_parks_.at(car)).count()*cost_per_second_;
        }
        if(now_parked_.count(car) > 0){
            price += chrono::duration_cast<chrono::seconds>(Clock::now() - now_parked_.at(car)).count()*cost_per_second_;
        }
        return price;
    }
    ...
    unordered_map<VehiclePlate, int64_t, VehiclePlateHasher> EndPeriodAndGetBills() {
        ...
        for(const auto& [car, _]: complete_parks_) {
            int64_t price = GetCurrentBill(car);
            if(price > 0) {
                result[car] = price;
                if(now_parked_.count(car) > 0) {
                    now_parked_[car] = Clock::now();
